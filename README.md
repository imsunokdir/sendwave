...
│   └── Dockerfile
│
└── docker-compose.yml       # Runs server + client + Redis
```
...
No need to install Redis or Node.js separately.
...
# Redis (production only — Railway provides this automatically)
...
### 4. Auto-Reply
Lead replies → IMAP cron detects it → classified by AI → RAG pipeline generates reply using your knowledge base → sent automatically.
...
- **HuggingFace API over local models** — Avoids Node.js heap memory issues from loading ML models in-process (~80% memory reduction)
...
- **Environment-aware Redis** — Automatically uses local Redis in development, switches to Railway Redis in production via `REDIS_URL`
- **Docker multi-stage builds** — Separate build and production stages keep images lean (server: ~622MB, client: ~94MB)
